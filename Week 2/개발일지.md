...
### 테이블 생성
    ...
    SHOW TABLES; = 현재 데이터베이스가 가진 모든 테이블들을 조회
    SHOW COLUMNS FROM <table_name>; = 해당 이름의 테이블이 가진 칼럼값들을 조회
    DESC <table_name>; = 역시 똑같은 기능
    -- : 주석
    DROP TABLE <table_name>; = 해당 이름의 테이블을 삭제
